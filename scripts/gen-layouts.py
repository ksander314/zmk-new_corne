...
NOT have to come from the host is the *arrangement*. Before this, it did:
switching to Russian meant falling back to the stock ``ru`` layout, which is
laid out for QWERTY positions, so the firmware had to hop to a QWERTY layer to
match -- and a keyboard that lost track of the host then typed nonsense.

These layouts remove that. They are the stock Russian layout permuted so that
every Cyrillic letter lands on the same *physical* key it has always occupied,
while the firmware types DVP in both languages. The keyboard still knows which
language is on, because four punctuation combos need a different scan code per
host layout, but that state no longer touches the alphabet: forgetting to
switch costs those four combos rather than every letter.

How the permutation is derived
------------------------------
For a physical key P, the stock Russian layout produces ``ru[qwerty(P)]``,
where ``qwerty(P)`` is the key an ANSI board carries at that position. With the
firmware typing DVP the host instead receives ``dvp(P)``, so the new layout must
satisfy::

    layout[dvp(P)] = ru[qwerty(P)]

``dvp`` is read straight out of the keymap, so retuning the letters and
re-running this script keeps the two in step. ``qwerty`` is the constant
``QWERTY_REF`` below: it describes the keyboard ``ru`` was drawn for, not this
one, and there is no QWERTY layer left to read it from. The mapping is asserted
to be a bijection over the alphanumeric block, and the two language modes are
asserted to carry identical letters, so a keymap that grows, blanks or retunes
a key fails generation instead of emitting a layout that silently drops a
letter.
...
# Binding index -> the key an ANSI QWERTY board carries at that position, i.e.
# the keyboard the stock `ru` layout was drawn for. The array is row-major and
# includes the joystick/encoder cluster in the middle of each row, hence the
# gaps; the same numbering is what the combo definitions in the keymap use.
#
#   row 1: 0 = outer left,  6 = joystick up,     12 = outer right
#   row 2: 13 = outer left, 19..21 = joystick,   27 = outer right
#   row 3: 28 = outer left, 34..35 = encoder,    41 = outer right
QWERTY_REF = {
    1: "AD01", 2: "AD02", 3: "AD03", 4: "AD04", 5: "AD05",
    7: "AD06", 8: "AD07", 9: "AD08", 10: "AD09", 11: "AD10", 12: "AC11",
    14: "AC01", 15: "AC02", 16: "AC03", 17: "AC04", 18: "AC05",
    22: "AC06", 23: "AC07", 24: "AC08", 25: "AC09", 26: "AC10",
    29: "AB01", 30: "AB02", 31: "AB03", 32: "AB04", 33: "AB05",
    36: "AB06", 37: "AB07", 38: "AB08", 39: "AB09", 40: "AB10",
}

# Letters no position can carry, because the key that would hold them is
# `&none` -- this keymap blanks both outer columns. They keep a scan code the
# base layer never sends, which is exactly where the symbol layer reaches them:
# <AB10> is the `/` on SPEC. Spelled out rather than inferred, so that blanking
# one more key fails generation instead of moving a letter somewhere silently.
ORPHANS = {"э": "AB10"}
...
def build_permutation():
    """Map each xkb key the base layer sends to the symbols it must produce."""
    text = KEYMAP.read_text(encoding="utf-8")
    en = parse_layer(text, "layer_0")
    ru = parse_layer(text, "layer_1")
    if en != ru:
        where = [i for i, (a, b) in enumerate(zip(en, ru)) if a != b]
        sys.exit("layer_0 and layer_1 must carry the same letters -- a language "
                 "mode is not a different alphabet -- but they differ at %s"
                 % (where or "different lengths"))

    mapping = {}
    orphans = {}
    for idx, source in sorted(QWERTY_REF.items()):
        if en[idx] == "none":
            orphans[RU[source][0]] = RU[source]
            continue
        target = xkb_key(en, idx, "layer_0")
        if target in mapping:
            sys.exit("two positions both send <%s>" % target)
        mapping[target] = RU[source]

    if set(orphans) != set(ORPHANS):
        sys.exit("blank positions strand %s, but ORPHANS covers %s"
                 % (sorted(orphans) or ["nothing"], sorted(ORPHANS)))
    for char, pair in orphans.items():
        target = ORPHANS[char]
        if target in mapping:
            sys.exit("<%s> is the fallback for %s, but the base layer already "
                     "sends it" % (target, char))
        mapping[target] = pair
...
def emit_xkb(mapping):
    lines = [
        ...
        "// Do not edit by hand -- re-run the generator after retuning letters.",
        "//",
        "// Pair this with the firmware typing DVP: every Cyrillic letter then",
        "// sits on the same physical key it occupies on any typewriter-pair",
        "// board, so switching language never has to move the alphabet.",
        "//",
